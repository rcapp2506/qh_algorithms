...
print(f"  Ratio CCNN/QCNN:    {ccnn_total / qcnn_total:>12.1f}×")
print()
...
print(f"  → CCNN trunk has ~{(ccnn_conv1+ccnn_conv2+ccnn_conv3+ccnn_conv4) / (qcnn_conv1+qcnn_bn1+qcnn_conv2+qcnn_bn2+qcnn_quanv_input_scale+qcnn_quanv_weights):.0f}× more trunk params than QCNN")
print()
print(f"  QCNN head (fc1+fc2):           {qcnn_fc1+qcnn_fc2:>10,}")
print(f"  CCNN head (fc1+fc2):           {ccnn_fc1+ccnn_fc2:>10,}")
print(f"  → CCNN head has ~{(ccnn_fc1+ccnn_fc2) / (qcnn_fc1+qcnn_fc2):.0f}× more head params than QCNN")
print()
print(f"  QCNN flatten input dim:        {flat:>10,}  (6 channels × 14×14)")
print(f"  CCNN flatten input dim:        {ccnn_flat:>10,}  (64 channels × 16×16)")
print(f"  → CCNN sees {ccnn_flat / flat:.1f}× more features at the linear head")
print()
